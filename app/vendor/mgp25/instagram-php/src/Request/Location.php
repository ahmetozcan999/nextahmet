...
class Location extends RequestCollection
{
        ...
        $maxId = null)
    {
        ...
        if ($maxId !== null) { // switch on pagination
            $locationFeed->addPost('max_id', $maxId);
        }
		
		// convert new response to old one
		...
		echo 'Recent Items:'.$mediaCounter."<br>";	

		// Call IG again for ranked items,  but only if this isn't a paginated call
		if ($maxId == null) {
			$locationFeed = $this->ig->request("locations/{$locationId}/sections/")
				->setSignedPost(false)
				->addPost('rank_token', $rankToken)
